...
const SYSTEM_PROMPT = `You are a nutrition assistant. The user tells you how many calories and grams of protein, carbs, and fat they still need today. Return ONLY a valid JSON array of exactly 3 meal suggestions. No markdown, no explanation — just the raw JSON array.
...
- Offer variety across the 3 suggestions (different cuisines, protein sources, or styles).
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const userMessage = `remaining: calories=${Math.round(remaining.calories)}, protein=${Math.round(remaining.protein)}g, carbs=${Math.round(remaining.carbs)}g, fat=${Math.round(remaining.fat)}g`;
